...
#   - Les CSV produits NE SONT PAS dans Git (gitignorés : Tests/Fixtures/Database/)
#   - Alternative hors-ligne : ./Tests/Scripts/generate_fixtures.sh
#
# ✅ Filtre deleted=0 automatiquement sur toutes les tables (soft-deletes TYPO3)
# ✅ Extrait uniquement les données représentatives (~50 Ko vs 1,3 Go)
# ✅ Anonymise emails, téléphones, noms, slugs, bodytexts
# ✅ Génère les fe_users synthétiquement (jamais extraits de la prod)
# ✅ Réassigne les UIDs production → UIDs stables 1–10
#
# Prérequis :
...
echo -e "${BLUE}ℹ️  deleted=0 forcé sur toutes les requêtes (soft-deletes filtrés)${NC}"
echo -e "${BLUE}ℹ️  Taille cible : ~50 Ko (vs 1,3 Go en production)${NC}"
echo ""

if ! command -v ddev &> /dev/null; then
    echo -e "${RED}❌ DDEV introuvable.${NC}"
    echo "   Alternative hors-ligne : ./Tests/Scripts/generate_fixtures.sh"
    exit 1
fi
...
if [ "${REMAINING:-0}" -gt 100 ]; then
    ...
    echo "   Lancez d'abord : ./Tests/Scripts/cleanup_database.sh"
    echo ""
fi
...
extract_table() {
    ...
    # Forcer deleted=0 si la table a ce champ et qu'il n'est pas déjà filtré
    ...
    if [ -s "$output" ]; then
        local lines=$(( $(wc -l < "$output") - 1 ))
        echo -e "  ${GREEN}✓${NC} $table — ${lines} lignes (soft-deletes filtrés)"
    else
...
# =============================================================================
# anonymize_csv : anonymise toutes les données sensibles via PHP
# =============================================================================
anonymize_csv() {
    local file=$1
    local table=$2
...
foreach (\$lines as \$line) {
    ...
    // Patterns sensibles dans TOUS les champs (filet de sécurité)
    foreach (\$d as \$k => \$v) {
...
reassign_uids() {
    ...
    echo -e "  ${GREEN}✓${NC} UID réassigné : $old_uid → $new_uid"
}

# =============================================================================
# SHARED — FE groups (extraits) + FE users (SYNTHÉTIQUES — jamais depuis prod)
# Les fe_users ne sont JAMAIS extraits de la production.
# Créés de zéro avec des données entièrement fictives.
# =============================================================================
...
# Récupérer les UIDs de groupes disponibles pour assigner aux fe_users test
G1=$(ddev mysql -e "SELECT uid FROM fe_groups WHERE hidden=0 AND deleted=0 ORDER BY uid LIMIT 1" \
    --batch --skip-column-names 2>/dev/null | tr -d '\n')
G2=$(ddev mysql -e "SELECT uid FROM fe_groups WHERE hidden=0 AND deleted=0 ORDER BY uid LIMIT 1 OFFSET 1" \
    --batch --skip-column-names 2>/dev/null | tr -d '\n')
G1=${G1:-1}; G2=${G2:-2}

# FE users : entièrement synthétiques, jamais extraits de la production
...
CSV
echo -e "  ${GREEN}✓${NC} shared/fe_users.csv — synthétiques (uid=100/101/102, mdp: password)"
echo ""
...
if [ -n "$CT_UIDS" ] && [ "$CT_UIDS" != "NULL" ]; then
    ...
    F_UIDS=$(ddev mysql -e \
        "SELECT GROUP_CONCAT(uid_local) FROM sys_file_reference \
         WHERE uid_foreign IN ($CT_UIDS) AND tablenames='tt_content' AND deleted=0" \
        --batch --skip-column-names 2>/dev/null | tr -d '\n')
    if [ -n "$F_UIDS" ] && [ "$F_UIDS" != "NULL" ]; then
        extract_table "sys_file" "uid IN ($F_UIDS)" \
            "$FIXTURE_DIR/page_with_images/sys_file.csv"
    fi
fi
...
if [ -n "$CAT_CT_UIDS" ] && [ "$CAT_CT_UIDS" != "NULL" ]; then
    ...
    CAT_UIDS=$(ddev mysql -e \
        "SELECT GROUP_CONCAT(uid_local) FROM sys_category_record_mm \
         WHERE uid_foreign IN ($CAT_CT_UIDS)" \
        --batch --skip-column-names 2>/dev/null | tr -d '\n')
...
echo -e "  ${GREEN}✓${NC} fe_groups.csv + fe_users.csv (synthétiques) copiés"
